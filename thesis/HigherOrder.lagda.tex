...
Following the approach by \citeauthor{DBLP:conf/haskell/WuSH14} we will define the
type class \AgdaDatatype{Syntax} for effects.
...
Because we have to lift programs as well as data, this would mean \AgdaDatatype{Set₁}.
...
lift values to \AgdaDatatype{Set} and programs to \AgdaDatatype{Set₁}.
...
Both programs produce results of type \AgdaArgument{X}, which is defined
using the context \AgdaFunction{Excᶜ}
...
\AgdaArgument{κ} and \AgdaArgument{σ} into the context.
