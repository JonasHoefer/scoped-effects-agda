...
open import Function using (_∘_; id; case_of_; const)
open import Level using (Level; 0ℓ; suc; Lift; lift; Setω)
...
%   hmap α (s , pf) = s , α ∘ pf
% 
% -- emap : {C : Container} {F : Set₁ → Set₁} {X Y : Set₁} → (F X → F Y) → ⟦ C ⟧ F X → ⟦ C ⟧ F Y
% -- emap {C} {F} {X} {Y} f (s , pf) = s , λ p → helper s p (pf p)
% --   where helper : (s : Shape C) (p : Pos C s) →
% --           F (fromMaybe X (Ctx C s p)) → F (fromMaybe Y (Ctx C s p))
% --         helper s p x with Ctx C s p
% --         ... | nothing = f x
% --         ... | just _  = x
% \end{code}
...
% 
% Using our choice of functor representation we can define the monad for higher
% order syntax.
...
For simplicity we will work with Bunkenburg's indexed Bi-Containers, because
they model effects with a simple continuation accurately.
In a more complex implementation, where more control over the type of the
continuation is needed, an indexed container producing a container might be
needed.

First we will port over the infrastructure for modular effects.
The coporduct carries over seamlessly.
We combine the position functions pairwise and handle the \AgdaFunction{Ctx}
functions also using a coproduct mediator.

\begin{code}[hide] % TODO: move
...
\end{code}
To implement bind we will define the \AgdaFunction{emap} function.
...
function can be defined generically for all effects.
...
we can define \AgdaFunction{emap} separately without hiding the termination.
...
The \AgdaFunction{inj} and \AgdaFunction{prj} functions have to be modified
...
pattern Throw e      = impure (inj₁ (throwˢ e) , _)
pattern Catch σ κ    = impure (inj₁ (catchˢ _) , κ , σ)
pattern Other s σ κ  = impure (inj₂ s , κ , σ)
\end{code}
...
\begin{code}
runExc (Catch σ κ)  = runExc (σ mainᵖ) >>= λ where
...
We call \AgdaFunction{handle} the given container extension, constructed using
\AgdaArgument{s}, \AgdaArgument{κ} and \AgdaArgument{σ}.
...
\begin{code}
runExc (Other s σ κ) = impure (handle (inj₂ tt) [ pure ∘ inj₁ , runExc ] (s , κ , σ))
\end{code}
