...
In this section we mention two ideas, one for each approach.
...
values, the changes to the context are not simple substitutions with  value,
but described by a function.
...
\AgdaDatatype{Set}.
These special data structures are only to large, because they contain values of
type \AgdaDatatype{Prog}, not arbitrary values from \AgdaDatatype{Set₁}.
...
\AgdaArgument{σ}.
As in Chapter \ref{chapter:first-order} and explained in Section
