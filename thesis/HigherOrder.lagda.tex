...
\citeauthor{DBLP:conf/haskell/WuSH14} introduce a second approach using
...
could be transferred to Agda using container representations for functors
and sized types.
...
\begin{itemize}
  \item discussion of different representations for higher order syntax
  \item combination of bounded universe levels and existential (stored) types
    problematic when modeling deep effects $\Rightarrow$ call-time choice cannot
    be modelled adequately (cannot model ambient nondeterminism in data
    structures)
  \item most simple 'fixes' for the problem are problematic
  \item key points of implementation without deep effects (mighty be useful for
    some readers)
\end{itemize}