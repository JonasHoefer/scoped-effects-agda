...
Algebraic operations are those which commute with \AgdaFunction{>>=} i.e. for an
$n$-ary algebraic operations the following holds.
...
\texttt{fork} this does obviously not hold.
They are not algebraic but can be represented as an effect handler.
Representing these operations as a handler moves the syntactic problem of
scoping to the level of the handler i.e. to a semantics level.
Decoupling the two leads to the idea of scoped effects.
