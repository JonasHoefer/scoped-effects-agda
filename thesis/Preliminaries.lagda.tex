...
ill-formed, for example circular arguments. % cite?
...
the \AgdaDatatype{Size<\_} family of type\footnote{We will not need this
  construction. It is usually used when working with \AgdaKeyword{coinductive}
  data types.}.
...
\AgdaFunction{fmap} better to assist the termination checker.
...
\AgdaFunction{fmap}, which is transparent for the termination checker, to
...
Examples include I/O, exceptions, nondeterminism, state and
...
alternative to monads for describing computational effects and capture a wide
...
discuss how well these two approaches translate from Haskell to Agda.
