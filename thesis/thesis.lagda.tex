...
\begin{abstract}
  ...
  algebraic effects.
  To allow development of modular programs and local reasoning in formal
  verification, the composability of representations is important.
  ...
  partiality, in the proof assistants Coq.

  In the traditional approach, operations with scope can only be implemented as
  an effect handler.
  ...
  complex effects, such as sharing.
  % Furthermore, to simulate sharing of potentially effectful programs, we need
  ...
  Our main example for a scoped effect is sharing of nondeterminism, as described
  by \textcite{bunkenburg2019modeling}, which was developed for simulating
  Curry's call-time choice semantics.
...
\begin{abstract}
  ...
  Um modulare Entwicklung von Programmen und Folgerung über Stücken von
  Programmen zu erlauben, ist die Modularität der Repräsentation entscheidend.
  Algebraische Wirkungen und Wirkungs-Behandler sind generell ein modularer
  ...
  Zum Beispiel haben
  \textcite{DBLP:journals/programming/DylusCT19,DBLP:conf/haskell/ChristiansenDB19}
  diese Verwenden um ambiente Wirkungen, wie Haskells Parzialität, im
  ...
  da diese nötig sind, um komplexere Nebenwirkungen wie Sharing zu modellieren.
  ...
  Wirkungen, welche verwendet werden können um Curry's~\cite{Hanus95curry}
  ambienten, nicht strikten Nichtdeterminismus zu modellieren.
  Unser Hauptbeispiel für einen Wirkung mit Sichtbarkeitsbereich ist Sharing von
  Nichtdeterminismus nach \textcite{bunkenburg2019modeling}, welche verwendet
  werden kann um Curry's Call-Time Choice Semantik zu modellieren.
