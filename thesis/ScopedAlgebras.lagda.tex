...
Furthermore, we presents a naive idea for modularisation of the new handlers.
...
proving the monad laws\footnote{As in chapter \ref{chapter:higher-order} the
  ...
  terms.}.
Instead we will define a generic \AgdaFunction{fold}, which we reuse for all
...
second monad.
In \citetitle{DBLP:conf/lics/PirogSWJ18} they show that the two monads are
...
because after calling the \AgdaDatatype{Share} handler no scopes are duplicated.
If such a situation should arrise the ID passed around by the handler can simply
be made optional via \AgdaDatatype{Maybe}, removing IDs from outside choices.
...
\textcite{DBLP:journals/corr/abs-1806-05230}.
Our recursion scheme, based on \textcite{DBLP:journals/corr/abs-1806-05230} idea
of introducing an index type, has essentially the same shape.

Using the recursion scheme we were able to define nondeterminism example by
