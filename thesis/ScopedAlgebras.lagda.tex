...
This chapter transfers the basic implementation by
...
Furthermore, we presents a new implementation for naive modularisation of
handlers.
...
\AgdaField{Ctx} was used to provide computations returning the result type as
...
Because container extensions are functor we \AgdaFunction{map} over the given
value.
...
second monad.
en \citetitle{DBLP:conf/lics/PirogSWJ18} they show that the two monads are
...
\begin{code}
  (                (x : A)                                        → P 0 x                     )  →
\end{code}
The \AgdaInductiveConstructor{var} can be handled similar to the examples by
\textcite{DBLP:journals/corr/abs-1806-05230}.
...
\AgdaInductiveConstructor{var}\AgdaSpace{}\AgdaArgument{x} therefore is part of
...
\AgdaField{a}.
Notice that in the definition of \AgdaFunction{fold} they are guarantied an index
...
outside id.
Scopes of foreign effects do not effect sharing.
The foreign scope is a subscope of the current sharing scope.
Therefore, the choices are labeled with the same scope id.
To guaranty the uniquenes of ids we have to thread the current choice id through
the inner scope.
...
scope.
In case of a sharing scope we numebr choices inside the scope unsing the scopes
...
\end{code}
We introduce the helper function \AgdaFunction{runShare} to conviniently call
the actuall sharing handler.
...
We introduce the function \AgdaFunction{runCTC} to easily evaluate programms
