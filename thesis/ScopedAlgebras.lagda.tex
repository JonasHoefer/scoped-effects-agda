...
Therefore, the \AgdaInductiveConstructor{var} constructors at layer zero have to
...
It is still possible to implement \AgdaFunction{once} using a version of
\AgdaFunction{cut} and \AgdaFunction{call}.
...
\ref{chapter:scoped-algebras:monad-e}, the \AgdaDatatype{Prog} monad is
...
It simulates handler for \AgdaDatatype{Share} that passes additional parameters
around.
\AgdaFunction{runShare} supplies initial parameters and hides this internal
...
were able to prove propositions by induction on \AgdaDatatype{Prog}.
