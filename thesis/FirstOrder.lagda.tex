...
convenient syntax to extend the continuation.
\AgdaFunction{op} generates the \textit{generic operation} for any operation
symbol.
The generic operation for an operation symbol $\mathrm{op} : P \leadsto A$ is
the operation with \AgdaInductiveConstructor{pure} as continuation.
Therefore, it is a function, usually notated as $\overbar{op}$, of type $P
\rightarrow \mathrm{Free}_T(A)$, justifying the $\leadsto$-notation.
...
Based on Bunkenburg's code~\cite{bunkenburg2019modeling} we introduce a type
