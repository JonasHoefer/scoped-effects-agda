...
tail of \AgdaArgument{C} in its effect stack and potentially modify the type
...
Internally the handler would map the \AgdaInductiveConstructor{pure}
...
% Nondet -> all continuations ; more control -> flexible handler i.e.
...
\AgdaDatatype{Cut} following \cite{DBLP:conf/haskell/WuSH14} in Agda.
...
\begin{AgdaAlign}
% TODO: mention that this sis essentially a parameter passing handler?
...
add \AgdaDatatype{Size} annotations to the functions.
...
The argument of the continuation \AgdaArgument{c} is itself a program, it is the
...
\end{code}
\end{center}
Thanks to the \AgdaDatatype{Shareable} type class it is also possible to share
...
\begin{itemize}
  \item Implementation of usual algebraic effects and handlers
  \item Scoped effects using first approach by \textcite{DBLP:conf/haskell/WuSH14}
  \item size annotated free monad is strong enough to implement scope delimiter
  \item same problems as in Haskell i.e. missmatched scoped delimiters possible
  \item missmatched scope delimiters potentially complicate proofs
  \item implementation of sharing effect by \textcite{bunkenburg2019modeling} -
    can model call-time choice
\end{itemize}