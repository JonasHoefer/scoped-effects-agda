...
position functions\footnote{The function \AgdaFunction{[\_,\_]} corresponds to
  the Haskell function \mintinline{haskell}{either :: (a -> c) -> (b -> c) ->
    Either a b -> c}.}.
...
constructors, because \AgdaDatatype{Void} has no operations i.e. its
\AgdaField{Shape} type has no constructors.
...
\AgdaDatatype{Free} context after all other effects have been handled.
...
⁇-identʳ p = begin
  ...
  solutions p                                   ∎
\end{code}
Proofing the right idientity is more complicated, because \AgdaFunction{fail} is
