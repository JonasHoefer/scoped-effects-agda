...
We used an similar approach similar to the Idris effect library
...
Because the returntype is constructed using \AgdaFunction{sum} over the given
...
transform a program producing a value of type \AgdaArgument{A} to one producing
a value of type
\AgdaArgument{E}\AgdaSpace{}\AgdaFunction{⊎}\AgdaSpace{}\AgdaArgument{A},
either an exception or a result.
Internally the handler would map the \AgdaInductiveConstructor{impure}
...
\begin{AgdaAlign}
  ...
  result that is not \AgdaFunction{fail}.
...
⁇-identˡ q = begin
  runNondet (fail ⁇ q)                      ≡⟨⟩ -- definition of handler for ⁇
  _++_ <$> runNondet fail  <*> runNondet q  ≡⟨⟩ -- definition of handler for fail
  _++_ <$> pure []         <*> runNondet q  ≡⟨⟩ -- definition of <$>
  pure ([] ++_)            <*> runNondet q  ≡⟨⟩ -- definition of <*> / ap-ident-left
  [] ++_                   <$> runNondet q  ≡⟨⟩ -- definition of ++ / ++-ident-left
...
\begin{code}
  ...
  go = runState {_} {ops} s₀
\end{code}
Given an arbitrary inital state \AgdaArgument{s₀} and rest effect stack
...
another handler, implementing the non-algebraic operation, can be run again on
the program in scope.
...
task of scoping from the handler to the syntax and therefore allows a flexible
ordering of the handlers and the implementation of scoped operations.
...
In the following sections we will implement scoped effects and exemplary present
...
stages.
First we implement the traditional effect \AgdaDatatype{Cut}, which can be used
...
the programs in scope and evaluates the handler for \AgdaDatatype{Cut} on each
of them.
...
unexplored alternatives in its second argument using \AgdaFunction{⁇}.
\AgdaFunction{fail} is the neutral element for \AgdaFunction{⁇} and therefore
the default argument.
Since this handler is non-orthogonal (it interacts with another effects syntax)
...
To prove termination we mark the second argument with an arbitrary size
\AgdaArgument{i}.
...
This fact is curcial to prove that the recursive calls to \AgdaFunction{bcall} and
...
semantics are realised using sharing of nondeterministic choices.
...
associated effects.
Ignoring effects of certain components of the data structure is not possible
with simple lifted lists, i.e. values of type
\AgdaDatatype{Free}\AgdaSpace{}\AgdaArgument{ops}\AgdaSpace{}$($\AgdaDatatype{List}\AgdaSpace{}\AgdaArgument{A}$)$
...
The values stored by each \AgdaInductiveConstructor{∷ᴹ} and therefore there
effects are ignored.
...
! mx = mx >>= nf
\end{code}
The type class allows to normalize elements of type \AgdaArgument{A} (intuitively
...
The identifier is generated from the scope id of the current sharing scope and a
...
as done by \textcite{bunkenburg2019modeling} in his Coq implementation.
...
with the same id.
An in depth explanation can be found in \citetitle{bunkenburg2019modeling}.
...
It is the original \AgdaFunction{coin} function inside a sharing scope, which
